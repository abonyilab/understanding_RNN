# -*- coding: utf-8 -*-
"""
Created on Thu Oct 12 11:40:55 2017
...
def hyperellipsoid(P, y=None, z=None, pvalue=.95, units=None, show=True, 
                   ax=None, covar=None, variance=None, scatter_labels=None, savefig=None):
    """
    ...
    if show and (p == 2 or p == 3):
        _plot(P, hypervolume, saxes, center, R, pvalue, units, ax, scatter_labels, variance, savefig)

    return hypervolume, saxes, angles, center, R


def _plot(P, hypervolume, saxes, center, R, pvalue, units, ax, scatter_labels, variance, savefig):
...
#                    [x[i,j],y[i,j],z[i,j]] = np.dot([x[i,j],y[i,j],z[i,j]], R) + center
        ...
        if saxes.size == 2:
            ...
            if scatter_labels is not None:
                for k in range(len(scatter_labels)):
                    plt.text(P[k,0],P[k,1],scatter_labels[k])
            plt.axis('equal')
            plt.grid()
            ...
            if scatter_labels is not None:
                for k in range(len(scatter_labels)):
                    ax.text(P[k,0],P[k,1],P[k,2],scatter_labels[k])
            # plot ellipsoid
